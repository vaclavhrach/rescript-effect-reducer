...
module Mock = {
  ...
  // Update
  let update = (model, msg): (model, Cmd.t<msg>) =>
    switch msg {
    | Increment => ({...model, value: model.value + 1}, Cmd.none)
    | IncrementWithPayload(payload) => ({...model, value: model.value + payload}, Cmd.none)
    }
}
...
test("value should be initialized to \"0\" ", () => {
  let {result} = renderHook(() => useElmish({init: init, update: update}))
